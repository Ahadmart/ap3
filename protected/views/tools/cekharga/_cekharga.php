    ...
    function isiView(data) {
        ...
        while (i < len) {
            $("#view-harga").append('<br /><small>' +
                new Intl.NumberFormat('de-DE').format(data.hj_multi[i].harga * data.hj_multi[i].qty) + ' / ' + data.hj_multi[i].nama_satuan + ' (' + data.hj_multi[i].qty + ' pcs), @' + new Intl.NumberFormat('de-DE').format(data.hj_multi[i].harga) +
                '</small>')
            i++
        }
    }
