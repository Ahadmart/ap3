...
class AkmDetail extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('akm_id, barang_id, harga_jual', 'required'),
			array('akm_id, barang_id, qty', 'length', 'max'=>10),
			array('harga_jual, diskon', 'length', 'max'=>18),
			array('created_at, updated_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, akm_id, barang_id, qty, harga_jual, diskon, updated_at, created_at', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'akm' => array(self::BELONGS_TO, 'Akm', 'akm_id'),
		);
	}
