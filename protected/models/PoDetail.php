 ...
 * @property string $harga_beli_terakhir
 * @property double $ads
...
class PoDetail extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('po_id, barcode, nama, harga_beli_terakhir, updated_at, updated_by', 'required'),
			array('stok, saran_order, status', 'numerical', 'integerOnly'=>true),
			array('ads, est_sisa_hari', 'numerical'),
			array('po_id, barang_id, qty_order, updated_by', 'length', 'max'=>10),
			array('barcode', 'length', 'max'=>30),
			array('nama', 'length', 'max'=>45),
			array('harga_beli_terakhir', 'length', 'max'=>18),
			array('created_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, po_id, barang_id, barcode, nama, harga_beli_terakhir, ads, stok, est_sisa_hari, saran_order, qty_order, status, updated_at, updated_by, created_at', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('harga_beli_terakhir',$this->harga_beli_terakhir,true);
