...
class PembelianPpn extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('pembelian_id, total_ppn_hitung, total_ppn_faktur, updated_at, updated_by', 'required'),
			array('status', 'numerical', 'integerOnly'=>true),
			array('pembelian_id, updated_by', 'length', 'max'=>10),
			array('no_faktur_pajak', 'length', 'max'=>45),
			array('total_ppn_hitung, total_ppn_faktur', 'length', 'max'=>18),
			array('created_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, pembelian_id, no_faktur_pajak, total_ppn_hitung, total_ppn_faktur, status, updated_at, updated_by, created_at', 'safe', 'on'=>'search'),
		);
	}
